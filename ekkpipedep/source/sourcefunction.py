# -*- coding: utf-8 -*-
from typing import Union, List, Callable, Optional, Any, Tuple, Dict
import warnings
...
MineralReaction = Any #TODO : Find the actual type here
...
class StationaryPipeSourceFunction():
    def __init__(self,
        ...
        pressure : Pressure, optional
            Pressure. The default is constants.ATM.
    ...
    def f(self, t : float, y : np.ndarray) -> np.ndarray: #Main source function
        ...
        input_dict = self.split_input(y) #Split input in ['moments', 'elements']
        output_dict = self.split_input(np.zeros_like(y)) #Make dictionary to hold outputs
        ...
        elements_balance = self.get_elements_balance_dict(input_dict['elements'], TKb) #Get elements balance
        vrepr = moments[1]/moments[0]
        
        pressure = self.pressure_function(pos) #Pressure at bulk
        sol_bulk = self._solve_bulk_equilibrium(elements_balance, TKb, pressure) #Solve bulk chemical equilibrium
        ...
        if self.has_agglomeration or self.has_growth or self.has_particle_deposition:
            if len(self.bulk_phases) > 1:
                raise NotImplementedError("We currently cannot model particles with more than one mineral phase")
        ...
        if self.has_nucleation:
            ...
            output_dict['elements'] += nucleation_elements_source_vector #Get sourcce function for elements
            output_dict['moments'] += nucleation_moments_source_vector #Get source function for momentts
        ...
        if self.has_agglomeration:
            agglomeration_moments_source_vector = self.agglomeration_sources(vols,
                                                                             ...
                                                                             ionic_strength,
                                                                             vrepr)
            output_dict['moments'] += agglomeration_moments_source_vector #Agglomeration only influence moments
        ...
        output_vector = self.join_input(output_dict) #Join everything together
        ...
        self.make_temporary_record_from_dict(reactions_fluxes_dict, 'Jr_') #Reccord everything
    ...
    def ionic_deposition_elements_mass_sources(self, sol_wall : InterfaceSolution, 
        ...
        """
        wall_scale_params = 1/(flowproperties.pipe_flux_scale(pipe_diameter)) #1/m*(m^3/kg)
        ...
        for phase in self.wall_phases:
            ...
            source_mass += np.pi*pipe_diameter*molar_mass*sol_wall.reaction_fluxes[phase]
    ...
    def agglomeration_sources(self, vols : np.ndarray, weights : np.ndarray,
        ...
        if interactions:
            if len(self.bulk_phases) > 1:
                warnings.warn("More than one bulk phase. Assuming properties of the first one")
            phase = self.bulk_phases[0]
            phase_properties = properties.get_solid_phase_properties(phase)
        hamaker = None if not interactions else phase_properties.get_hamaker_constant(TK)
        debye_length = None if not interactions else phase_properties.get_debye_length(ionic_strength, TK)
        phi_dl = None if not interactions else phase_properties.double_layer_potential
        permittivity = None if not interactions else constants.WATER_PERMITTIVITY
    ...
    def growth_sources(self, vols : np.ndarray, weights : np.ndarray,
        ...
        elements_balance_vector = element_balance_vector[2:-1, :]
        k_aux = self.kmoments[1:].reshape(-1, 1) #(n-1, 1) #zero-th moment source will be zero by definition
        source_moments_growth_ = (k_aux*growth_rate_vector*vols**(k_aux - 1)*weights).sum(axis=-1) #(n-1,)
        ...
        for i, phase in enumerate(self.bulk_phases):
            ...
            source_elements_phase_growth = elements_balance_vector[:, i]*molar_vol*np.sum(phase_growth_rate_vector*weights).sum()
    ...
    def single_bulk_phase_properties(self) -> Tuple[float, float]:
        if len(self.bulk_phases) > 1:
            warnings.warn("More than one bulk phase. Using the first one as representative")
        phase = self.bulk_phases[0]
        phase_properties = properties.get_solid_phase_properties(phase)
        molar_mass = phase_properties.molar_mass
        density = phase_properties.density
